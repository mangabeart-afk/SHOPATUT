            ...
            {visibleRows.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <td>{row.series || '—'}</td>
                  <td><div className="article-detail-stack"><strong>{row.detail || '—'}</strong>{[row.seller, row.origin, row.notes].filter((value) => value?.trim()).length > 0 ? (<span>{[row.seller, row.origin, row.notes].filter((value) => value?.trim()).join(' · ')}</span>) : null}</div></td>

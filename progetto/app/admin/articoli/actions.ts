...
export async function deleteSelectedArticles(formData: FormData) {
  ...
  for (const id of ids) {
    const { error } = await supabase.rpc('admin_delete_article', { p_article_id: id })
    if (error) redirect(`/admin/articoli/archivio?error=${encodeURIComponent(error.message)}`)
  }

  redirect('/admin/articoli/archivio?message=Articoli eliminati correttamente. Le vendite collegate sono state annullate e lo storico conservato.')
}

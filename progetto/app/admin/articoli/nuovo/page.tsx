...
export default async function NuovoArticoloPage() {
  ...
  const { data: customers } = await supabase
    .from('customers')
    .select('id, first_name, last_name, email, mailboxes(mailbox_code)')
    .order('last_name')
    .order('first_name')
  ...
  const normalizedCustomers = (customers || []).map((customer: any) => ({
    ...
    mailbox_code: Array.isArray(customer.mailboxes) ? customer.mailboxes[0]?.mailbox_code || null : customer.mailboxes?.mailbox_code || null,
  }))
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <section className="panel">
          <ArticleForm action={createArticle} customers={normalizedCustomers as any} />
        </section>
      </main>
    </div>
  )

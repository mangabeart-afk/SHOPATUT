...
BEGIN
  ...
  SELECT mailbox_code INTO v_old_code
  ...
  -- Retroattività: i movimenti che memorizzano il codice cliente devono
  -- mostrare il nuovo codice anche per le operazioni già registrate.
  UPDATE public.movements
  SET reference_code = v_code
  WHERE mailbox_id = p_mailbox_id
    AND movement_type = 'NUOVO_UTENTE'
    AND (reference_code = v_old_code OR reference_code IS NULL);

  UPDATE public.movements
  SET generic_customer_name = v_code
  WHERE mailbox_id = p_mailbox_id
    AND movement_type = 'VENDITA'
    AND (generic_customer_name = v_old_code OR generic_customer_name IS NULL);

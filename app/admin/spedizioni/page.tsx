...
async function createShipment(formData: FormData) {
  ...
  const shippingPaid = formData.get('shipping_paid') === 'on'
  ...
  const { data: assignmentRows, error: assignmentError } = await supabase
    ...
    .select('article_id,quantity_assigned,status,articles!inner(status)')
    .eq('mailbox_id', mailboxId)
    .eq('status', 'ATTIVA')
    .eq('articles.status', 'IN_STOCK')
  ...
  const { data: alreadyShipped } = await supabase
    .from('shipment_items')
    .select('article_id,quantity_shipped,shipments(status)')
    .eq('mailbox_id', mailboxId)
    .in('article_id', selectedArticleIds)
  const shippedByArticle = new Map<string, number>()
  for (const row of alreadyShipped || []) {
    const shipment = Array.isArray((row as any).shipments) ? (row as any).shipments[0] : (row as any).shipments
    if (shipment?.status === 'ANNULLATA') continue
    shippedByArticle.set(row.article_id, (shippedByArticle.get(row.article_id) || 0) + Number(row.quantity_shipped || 0))
  }
  ...
  for (const item of items) {
    const assigned = Math.max(0, (assignedByArticle.get(item.articleId) || 0) - (shippedByArticle.get(item.articleId) || 0))
  ...
  await notifyCustomer(mailboxId, 'SPEDIZIONE', 'Spedizione registrata', `È stata registrata una nuova spedizione ${shipment.shipment_code}.`, shipment.id)
  ...
  if (shippingCost > 0) {
    const { data: movement, error: movementError } = await supabase
      ...
      .insert({
        mailbox_id: mailboxId,
        movement_type: 'SPEDIZIONE',
        reference_id: shipment.id,
        reference_code: shipment.shipment_code,
        total_amount_eur: shippingCost,
        description: `Costo spedizione ${shipment.shipment_code}`,
    ...
    if (movementError || !movement) {
      redirect(`/admin/spedizioni?error=${encodeURIComponent(movementError?.message || 'Impossibile registrare il costo della spedizione.')}`)
    }
    ...
    if (shippingPaid) {
      const { data: payment, error: paymentError } = await supabase
        ...
        .insert({
          ...
          currency: 'EUR',
          exchange_rate: 1,
          payment_method: 'SPEDIZIONE',
          reference: shipment.shipment_code,
          status: 'RICEVUTO',
          notes: `Pagamento costo spedizione ${shipment.shipment_code}`,
          created_by: user.id,
        })
        .select('id,payment_code,amount_eur')
        .single()

      if (paymentError || !payment) {
        redirect(`/admin/spedizioni?error=${encodeURIComponent(paymentError?.message || 'Impossibile registrare il pagamento della spedizione.')}`)
      }
      ...
      await supabase.from('movements').insert({
        ...
        reference_code: payment.payment_code,
        total_amount_eur: Number(payment.amount_eur || shippingCost),
        description: `Pagamento spedizione ${shipment.shipment_code}`,
        operator_user_id: user.id,
        notes: `Riferimento spedizione ${shipment.shipment_code}`,
      })
      ...
      await notifyCustomer(mailboxId, 'PAGAMENTO', 'Pagamento registrato', `È stato registrato il pagamento di ${shippingCost.toFixed(2)} € relativo alla spedizione ${shipment.shipment_code}.`, payment.id)
    }
  }
...
async function cancelShipment(formData: FormData) {
  ...
  const { data: beforeCancel } = await supabase.from('shipments').select('mailbox_id,shipment_code').eq('id', id).maybeSingle()
  const { error } = await supabase.rpc('cancel_customer_shipment', { p_shipment_id: id })
  if (error) redirect(`/admin/spedizioni?error=${encodeURIComponent(error.message)}`)

  if (beforeCancel?.mailbox_id) await notifyCustomer(beforeCancel.mailbox_id, 'ANNULLAMENTO', 'Spedizione annullata', `La spedizione ${beforeCancel.shipment_code || ''} è stata annullata.`, id)
  redirect('/admin/spedizioni?message=Spedizione cancellata correttamente.')
}
...
async function updateShipment(formData: FormData) {
  ...
  const { data: previousShipment } = await supabase
    .from('shipments')
    .select('id,mailbox_id,shipment_code,shipping_cost_eur,status')
    .eq('id', id)
    .maybeSingle()

  if (!previousShipment) redirect('/admin/spedizioni?error=Spedizione non trovata.')

  if (status === 'ANNULLATA') {
    const { data: cancelTarget } = await supabase.from('shipments').select('mailbox_id,shipment_code').eq('id', id).maybeSingle()
    const { error: cancelError } = await supabase.rpc('cancel_customer_shipment', { p_shipment_id: id })
    if (cancelError) redirect(`/admin/spedizioni?error=${encodeURIComponent(cancelError.message)}`)
    if (cancelTarget?.mailbox_id) await notifyCustomer(cancelTarget.mailbox_id, 'ANNULLAMENTO', 'Spedizione annullata', `La spedizione ${cancelTarget.shipment_code || ''} è stata annullata.`, id)
    redirect('/admin/spedizioni?message=Spedizione annullata correttamente.')
  }
  ...
  const activeShippingAmount = status === 'ANNULLATA' ? 0 : shippingCost
  const { data: shipmentMovement } = await supabase
    .from('movements')
    .select('id')
    .eq('reference_id', id)
    .eq('movement_type', 'SPEDIZIONE')
    .maybeSingle()

  if (activeShippingAmount > 0) {
    if (shipmentMovement) {
      await supabase.from('movements').update({
        mailbox_id: previousShipment.mailbox_id,
        total_amount_eur: activeShippingAmount,
        movement_at: new Date().toISOString(),
        notes: notes || null,
      }).eq('id', shipmentMovement.id)
    } else {
      await supabase.from('movements').insert({
        mailbox_id: previousShipment.mailbox_id,
        movement_type: 'SPEDIZIONE',
        reference_id: id,
        reference_code: previousShipment.shipment_code,
        total_amount_eur: activeShippingAmount,
        description: `Costo spedizione ${previousShipment.shipment_code}`,
        operator_user_id: user.id,
        notes: notes || null,
      })
    }
  } else if (shipmentMovement) {
    await supabase.from('movements').update({ total_amount_eur: 0 }).eq('id', shipmentMovement.id)
  }
...
export default async function AdminSpedizioniPage({
  ...
  const articleIds = Array.from(new Set((assignments || []).map((row: any) => row.article_id)))
  let articlesForShipment: Array<{ id: string; article_code: string; detail: string | null; mailbox_id: string; available: number }> = []
  if (articleIds.length) {
    const [{ data: articleRows }, { data: shippedRows }] = await Promise.all([
      supabase.from('articles').select('id,article_code,detail,status').in('id', articleIds).eq('status','IN_STOCK'),
      supabase.from('shipment_items').select('article_id,mailbox_id,quantity_shipped,shipments(status)').in('article_id', articleIds),
    ])
    const shippedByKey = new Map<string, number>()
    for (const row of shippedRows || []) {
      const shipment = Array.isArray((row as any).shipments) ? (row as any).shipments[0] : (row as any).shipments
      if (shipment?.status === 'ANNULLATA') continue
      const key = `${row.mailbox_id}:${row.article_id}`
      shippedByKey.set(key, (shippedByKey.get(key) || 0) + Number(row.quantity_shipped || 0))
    }
    for (const row of assignments || []) {
      const article = (articleRows || []).find((a: any) => a.id === row.article_id)
      if (!article) continue
      const key = `${row.mailbox_id}:${row.article_id}`
      const available = Number(row.quantity_assigned || 0) - (shippedByKey.get(key) || 0)
      if (available > 0) articlesForShipment.push({ id: article.id, article_code: article.article_code, detail: article.detail, mailbox_id: row.mailbox_id, available })
    }
  }
  ...
  const visibleShipments = selectedStatus ? shipments : shipments.filter((s) => s.status !== 'CONSEGNATA')
    ...
    <main className="shell">
      ...
      <section className="content">
        ...
        <section className="panel">
          ...
          <ShipmentForm action={createShipment} mailboxes={normalizedMailboxes.map((m) => ({ id: m.id, mailbox_code: m.mailbox_code, customer_name: m.customer ? `${m.customer.first_name} ${m.customer.last_name}` : 'Cliente non trovato' }))} articles={articlesForShipment} />
        </section>

        <section className="panel">
          <h2>Ricerca e filtri</h2>
          <form action="/admin/spedizioni" method="get" className="compact-search-form shipment-filter-form">
            <label>
              Cerca
              <input
                type="search"
          ...
          {filteredShipments.length === 0 ? (
            ...
            <div className="movement-list">
              {filteredShipments.map((shipment) => {
                ...
                return (
                  <div className="shipment-list-row" key={shipment.id}>
                    <div>
                      <b>{shipment.shipment_code}</b>
                      <span>Casella: {mailbox?.mailbox_code ? <a href={`/admin/clienti/${mailbox.customer_id}`}>{mailbox.mailbox_code}</a> : '—'}</span>
                    ...
                    {shipment.status !== 'ANNULLATA' && <form action={cancelShipment} style={{ marginTop: 8 }}><input type="hidden" name="shipment_id" value={shipment.id} /><button type="submit" className="danger-button">Cancella spedizione</button></form>}
                  </div>
                )
              })}
            </div>

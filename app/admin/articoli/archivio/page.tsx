...
async function registerSale(formData: FormData) {
  ...
  const { data: saleMailbox } = await supabase.from('mailboxes').select('id,customer_id').ilike('mailbox_code', customerCode).maybeSingle()
  if (saleMailbox?.id) await notifyCustomer(saleMailbox.id, 'ACQUISTO', 'Acquisto registrato', `È stato registrato un nuovo acquisto per il tuo account per un totale di ${saleAmount.toFixed(2)} €.`, saleMailbox.id)
  if (paymentEnabled && saleMailbox?.id) await notifyCustomer(saleMailbox.id, 'PAGAMENTO', 'Pagamento registrato', `È stato registrato un pagamento di ${paymentAmount.toFixed(2)} € collegato alla vendita.`, saleMailbox.id)

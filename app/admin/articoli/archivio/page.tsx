...
export default async function ArchivioArticoliPage({ searchParams }: Props) {
  ...
  const rows = (articles || []).map((article: any) => {
    ...
    return { ...article, sold_quantity: sales.quantity, sales_revenue: sales.revenue, remaining_quantity: Math.max(0, Number(article.quantity_purchased || 0) - sales.quantity), mailbox_codes: Array.from(sales.customers) }
  })

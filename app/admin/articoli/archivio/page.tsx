...
export default async function ArchivioArticoliPage({ searchParams }: Props) {
  ...
  let query = supabase.from('articles').select('id,article_code,purchase_date,series,detail,origin,total_cost_eur,quantity_purchased,photo_url,status').order('purchase_date', { ascending: false })
  if (search) {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {error ? <section className="panel"><p className="error">Impossibile caricare gli articoli: {error.message}</p></section> : <ArticleArchiveTable rows={rows} initialSearch={search} />}
    </main>
  </div>
}

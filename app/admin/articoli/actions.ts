...
import { notifyNewArticleDigest, notifyCustomer } from '../../../lib/email'
...
export async function createArticle(formData: FormData) {
  ...
  await supabase.from('movements').insert({
    ...
    description: 'Articolo acquistato e registrato',
  ...
  if (text(formData, 'sale_enabled') === 'on') {
    ...
    const { data: saleMailbox } = await supabase.from('mailboxes').select('id').ilike('mailbox_code', customerCode).maybeSingle()
    if (saleMailbox?.id) await notifyCustomer(saleMailbox.id, 'ACQUISTO', 'Acquisto registrato', `È stato registrato un acquisto dell'articolo ${article.article_code}.`, article.id)
  }
...
export async function updateSelectedArticles(formData: FormData) {
  ...
  for (const id of ids) {
    ...
    if (photo instanceof File && photo.size > 0) {
      ...
      const path = `${id}-${Date.now()}.${extension}`
      const { error: uploadError } = await supabase.storage.from('article-photos').upload(path, photo, { contentType: photo.type, upsert: true })
      if (uploadError) redirect(`/admin/articoli/archivio?error=${encodeURIComponent(uploadError.message)}`)
      const { data: publicData } = supabase.storage.from('article-photos').getPublicUrl(path)
...
export async function deleteSelectedArticles(formData: FormData) {
  ...
  const { data: affectedAssignments } = await supabase.from('article_assignments').select('mailbox_id').in('article_id', ids)
  const affectedMailboxes = Array.from(new Set((affectedAssignments || []).map((r: any) => r.mailbox_id).filter(Boolean)))
  const { error } = await supabase.from('articles').delete().in('id', ids)
  if (error) redirect(`/admin/articoli/archivio?error=${encodeURIComponent(error.message)}`)
  for (const mailboxId of affectedMailboxes) await notifyCustomer(mailboxId, 'ANNULLAMENTO', 'Articolo annullato', 'Uno degli articoli collegati al tuo account è stato annullato.', ids[0])

...
export async function createArticle(formData: FormData) {
  ...
  if (photo instanceof File && photo.size > 0) {
    if (!photo.type.startsWith('image/')) redirect('/admin/articoli?error=Il file selezionato non è una foto valida.')
    if (photo.size > 8 * 1024 * 1024) redirect('/admin/articoli?error=La foto supera il limite di 8 MB.')
    ...
    if (uploadError) redirect(`/admin/articoli?error=${encodeURIComponent(uploadError.message)}`)

...
async function createCredit(formData: FormData) {
  ...
  await notifyCustomer(mailboxId, 'CREDITO', 'Credito registrato', `È stato registrato un credito di ${amount.toFixed(2)} € sul tuo account.`, createdCredit.id)
...
async function updateCredit(formData: FormData) {
  ...
  if (error) redirect(`/admin/crediti?error=${encodeURIComponent(error.message)}`)
  if (updatedCredit) {
    if (status === 'ANNULLATO') await notifyCustomer(updatedCredit.mailbox_id, 'ANNULLAMENTO', 'Credito annullato', 'Un credito del tuo account è stato annullato.', updatedCredit.id)
    else await notifyCustomer(updatedCredit.mailbox_id, 'CREDITO', 'Credito modificato', `Il credito del tuo account è stato aggiornato a ${amount.toFixed(2)} €.`, updatedCredit.id)
...
async function deleteCredit(formData: FormData) {
  ...
  const { data: creditBeforeDelete } = await supabase.from('credits').select('id,mailbox_id').eq('id', id).maybeSingle()
  const { data: deletedCredit, error } = await supabase.from('credits').delete().eq('id', id).select('id').maybeSingle()
  if (error) redirect(`/admin/crediti?error=${encodeURIComponent(error.message)}`)
  if (deletedCredit) await supabase.from('movements').delete().eq('reference_id', id).eq('movement_type', 'CREDITO')
  if (creditBeforeDelete?.mailbox_id) await notifyCustomer(creditBeforeDelete.mailbox_id, 'ANNULLAMENTO', 'Credito annullato', 'Un credito del tuo account è stato cancellato.', id)

...
export default async function AdminCreditiPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [{ data: mailboxes, error: mailboxError }, { data: credits, error }] = await Promise.all([
    supabase.from('mailboxes').select('id,mailbox_code,customer_id,status,customers(first_name,last_name)').order('mailbox_code'),
  ...
  const filtered = (credits || []).filter((credit: any) => {
    ...
    return [credit.credit_code, credit.status, credit.reason, credit.notes, mailbox?.mailbox_code, customer?.customer?.first_name, customer?.last_name].filter(Boolean).join(' ').toLowerCase().includes(search.toLowerCase())
  })

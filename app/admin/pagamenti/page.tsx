...
async function createPayment(formData: FormData) {
  ...
  const mailbox: any = (mailboxes || []).find((m: any) => {
    ...
    return [m.mailbox_code,c?.c?.first_name,c?.last_name,c?.email].filter(Boolean).some((v: any) => String(v).toLowerCase() === lookup.toLowerCase())
  })
...
export default async function AdminPagamentiPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const rows = (payments || []).filter((payment: any) => {
    ...
    return [payment.payment_code,payment.status,payment.payment_method,mailbox?.mailbox_code,c?.c?.first_name,c?.last_name,c?.email].filter(Boolean).join(' ').toLowerCase().includes(search)
  })

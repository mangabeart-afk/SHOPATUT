...
async function createPayment(formData: FormData) {
  ...
  const { data: mailboxes } = await supabase.from('mailboxes').select('id,mailbox_code,customer_id,customers(first_name,last_name,customer_code,email)').order('mailbox_code')
  const mailbox: any = (mailboxes || []).find((m: any) => {
    const c = Array.isArray(m.customers) ? m.customers[0] : m.customers
    return [m.mailbox_code,c?.customer_code,c?.first_name,c?.last_name,c?.email].filter(Boolean).some((v: any) => String(v).toLowerCase() === lookup.toLowerCase())
  })
...
export default async function AdminPagamentiPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [{ data: payments, error }, { data: mailboxes, error: mailboxError }, { data: allocations }, { data: movements }, { data: articles }] = await Promise.all([
    ...
    supabase.from('mailboxes').select('id,mailbox_code,customer_id,customers(first_name,last_name,customer_code,email)').order('mailbox_code'),
  ...
  const rows = (payments || []).filter((payment: any) => {
    ...
    return [payment.payment_code,payment.status,payment.payment_method,mailbox?.mailbox_code,c?.customer_code,c?.first_name,c?.last_name,c?.email].filter(Boolean).join(' ').toLowerCase().includes(search)
  })

...
async function createCustomerMailbox(formData: FormData) {
  ...
  const { data: mailboxByCustomer } = await supabase.from('customers').select('id').eq('email', payload.p_email).order('created_at', { ascending: false }).limit(1).maybeSingle()
  if (mailboxByCustomer?.id) {
    const { data: mailbox } = await supabase.from('mailboxes').select('id').eq('customer_id', mailboxByCustomer.id).order('opened_at', { ascending: false }).limit(1).maybeSingle()
    if (mailbox?.id) await notifyRegistration(mailbox.id)
  }
...
export default async function ClientiAdminPage({ searchParams }: PageProps) {
  ...
  const [{ data: customers }, { data: mailboxes }, { data: movements }, { data: assignments }, { data: shipmentItems }, { data: shipments }] = await Promise.all([
    ...
    supabase.from('movements').select('mailbox_id,movement_at,total_amount_eur,quantity,article_id,movement_type,reference_id'),
  ...
  const rows = customerRows.map((customer: any) => {
    ...
    const balance = movementRows
      .filter((r: any) => mailboxIds.has(r.mailbox_id))
      .reduce((sum: number, r: any) => sum + Number(r.total_amount_eur || 0), 0)
    ...
    const lastOrder = movementRows.filter((r: any) => mailboxIds.has(r.mailbox_id) && r.movement_type === 'VENDITA').map((r: any) => r.movement_at).sort().at(-1) || null
    return { customer, mailbox, balance, stock: Math.max(0, stock), incoming: Math.max(0, incoming), lastOrder }
  }).filter((row: any) => {
    if (!search) return true
    const text = [row.customer.first_name,row.customer.last_name,row.customer.email,row.customer.phone,row.mailbox?.mailbox_code].filter(Boolean).join(' ').toLowerCase()
        ...
        {rows.length === 0 ? <div className="empty">Nessun cliente trovato.</div> : <div className="movement-list">
          {rows.map((row: any) => <div className="customer-compact-row" key={row.customer.id}>
            <div className="customer-compact-grid customer-compact-grid-main">
              <span><b>CODICE CASELLA</b><a href={`/admin/clienti/${row.customer.id}`}>{row.mailbox?.mailbox_code || '—'}</a></span>
            ...
            <div className="customer-compact-grid customer-compact-grid-meta">
              ...
              <span><b>NOTE</b>{row.customer.notes || '—'}</span>
            </div>
          </div>)}
        </div>}
      </section>

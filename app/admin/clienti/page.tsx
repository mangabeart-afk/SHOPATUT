...
export default async function ClientiAdminPage({ searchParams }: PageProps) {
  ...
  const [{ data: customers }, { data: mailboxes }, { data: sales }, { data: payments }, { data: credits }, { data: assignments }, { data: shipmentItems }, { data: shipments }] = await Promise.all([
    ...
    supabase.from('movements').select('mailbox_id,movement_at,total_amount_eur,quantity,article_id,movement_type').in('movement_type',['VENDITA','SPEDIZIONE']),
  ...
  const rows = customerRows.map((customer: any) => {
    ...
    const salesTotal = salesRows.filter((r: any) => mailboxIds.has(r.mailbox_id) && r.movement_type === 'VENDITA').reduce((s: number, r: any) => s + Number(r.total_amount_eur || 0), 0)
    const shippingTotal = salesRows.filter((r: any) => mailboxIds.has(r.mailbox_id) && r.movement_type === 'SPEDIZIONE').reduce((s: number, r: any) => s + Number(r.total_amount_eur || 0), 0)
    ...
    const creditsTotal = (credits || []).filter((r:any)=>mailboxIds.has(r.mailbox_id)).reduce((s:number,r:any)=>s+Math.max(0,Number(r.amount_eur||0)-Number(r.used_amount_eur||0)),0)
    ...
    return { customer, mailbox, balance: salesTotal + shippingTotal - paymentsTotal - creditsTotal, stock: Math.max(0, stock), incoming: Math.max(0, incoming), lastOrder }
  }).filter((row: any) => {

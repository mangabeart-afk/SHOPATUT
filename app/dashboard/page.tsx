...
export default async function Dashboard() {
  ...
  return <main className="shell"><aside className="sidebar"><div className="brand">MangaBEART <span>[ShopaTüT]</span></div><nav>{['Dashboard','Caselle','Articoli','Pagamenti','Crediti','Spedizioni','Movimenti'].map((x,i)=><a className={i===0?'active':''} href={i===0?'/dashboard':`/${x.toLowerCase()}`} key={x}>{x}</a>)}</nav><div className="side-note">V1 • {role}<br/>{profile?.display_name || user.email}</div><SignOutButton/></aside>

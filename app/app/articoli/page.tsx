...
export default async function ArticoliPage() {
  ...
  const { data: articles, error } = await supabase
    .from('articles')
    .select('id,quantity_purchased,unit_cost_eur,total_cost_eur')
              ...
              {articles.map((article) => (
                <div className="movement" key={article.id}>
                  <div>
                    ...
                    <span>
                      Quantità: {Number(article.quantity_purchased || 0)}
                    </span>
                  </div>

                  <div>
                    <span>
                      {money(Number(article.unit_cost_eur || 0))} / unità
                    </span>
                    <strong>
                      {money(Number(article.total_cost_eur || 0))}
                    </strong>
                  </div>
                </div>
              ))}
